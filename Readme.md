...
以下命令可以快速启动最简单的StowDowner实例

- admin: `./admin -l 9999`
- agent: `./agent -c <admin's IP>:9999`
...
--tls 为节点通信启用TLS，在启用TLS后，AES加密将被禁用
...
- admin:  `./admin -l 9999` or `./admin -l 127.0.0.1:9999`

- agent:  `./agent -l 9999`  or `./agent -l 127.0.0.1:9999`
...
- admin:  `./admin -l 9999 -s 123`

- agent:  `./agent -l 9999 -s 123`
...
- admin:  `./admin -c 127.0.0.1:9999`

- agent:  `./agent -c 127.0.0.1:9999`
...
- admin:  `./admin -c 127.0.0.1:9999 --socks5-proxy xxx.xxx.xxx.xxx`

- agent:  `./agent -c 127.0.0.1:9999 --socks5-proxy xxx.xxx.xxx.xxx`

有用户名密码:

- admin:  `./admin -c 127.0.0.1:9999 --socks5-proxy xxx.xxx.xxx.xxx --socks5-proxyu xxx --socks5-proxyp xxx`

- agent:  `./agent -c 127.0.0.1:9999 --socks5-proxy xxx.xxx.xxx.xxx --socks5-proxyu xxx --socks5-proxyp xxx`
...
- admin:  `./admin -c 127.0.0.1:9999 --down ws`

- agent:  `./agent -c 127.0.0.1:9999 --up ws` or `./agent -c 127.0.0.1:9999 --up ws --down ws`
...
- agent-2:  `./agent -c 127.0.0.1:10000 --up ws`

第二，由于http是半双工的协议，并不是很适合StowDowner全双工的通信性质，所以这里http协议仅实现了http消息格式，并不是完整功能的http工作流，所以你仍然可以使用这个协议，但是StowDowner之间的流量在选择以http消息格式传输时无法通过nginx转发，这部分代码及功能保留一方面为了某些特殊情况下http协议的使用，另一方面也提供一个自定义流量的模版，方便用户自定义协议时拿来参考
...
- agent:  `./agent -c 127.0.0.1:9999 --reconnect 10`
...
示例如下
- admin: `./admin -l 10000 --tls -s 123`
- agent: `./agent -c localhost:10000 --tls -s 123`
...
示例如下
- admin: `./admin -l 10000 --tls -s 123`
- agent: `./agent -c xxx.xxx.xxx.xxx:10000 --tls -s 123 --domain xxx.com`
...
- admin: `./admin -l 8000 --tls -s 123 --down ws --heartbeat`
- agent: `./agent -c xxx.xxx.xxx.xxx:8080 --tls -s 123 --domain xxx.com --up ws`

## 端口复用机制

当前StowDowner提供基于SO_REUSEPORT和SO_REUSEADDR特性的端口复用功能及基于IPTABLES的端口复用功能
...
- SO_REUSEPORT和SO_REUSEADDR模式
  ...
  **主要支持windows、mac环境下的复用，linux亦可，但限制较多**
  ...
  - admin端：`./admin -c 192.168.0.105:80 -s 123`
  - agent端：`./agent  --report 80 --rehost 192.168.0.105 -s 123`
...
- IPTABLES模式
  ...
  **仅支持linux环境下的复用，agent会自动修改IPTABLES，需要root权限**
  ...
  - agent端： `./agent --report 22 -l 10000 -s 123`
  ...
  - 此时admin端就可以连接：`./admin -c 192.168.0.105:22 -s 123`
...
在StowDowner中，组成多级网络需要借助admin中的`listen`、`connect`、 `sshtunnel`命令来实现
...
- agent-1:  `./agent -c 127.0.0.1:9999 -s 123`

 此时用户还想连接agent-2，如下

- agent-2:  `./agent -l 10000 -s 123`
...
- agent-3: `./agent -c 127.0.0.1:10001 -s 123`
...
```
(admin) >> exit
[*] Do you really want to exit StowDowner?(y/n): y
[*] BYE!
```

当用户使用`use`命令选择了一个agent后，进入第二层node panel，其包含的命令如下

+ `generate`: 利用目标机器名，生成加密的agent连接命令

```
(admin) >> generate kali
[*] Agent Command: -c 1.86.240.94:9898 -b tcp
[*] Encrypt Agent Command: k2wiInjW0VYjFrrkxng1vTpPlZOhQpNnkcGHed+c4E4xkBl1x3UYRCQbal5L
```
...
(node 0) >> help
  ...
  exec       [command]                            Run a command in current node
...
- `exec`: 与当前节点建立交互式shell

```
(node 0) >> exec whoami
desktop-pga52ba\jerry
```
...
在严格受限的网络环境下，可以利用ssh隧道的方式来将StowDowner的流量伪装为ssh流量，从而避开防火墙的限制
...
https://xz.aliyun.com/t/10607?u_atoken=774b7ee3b0a288ed4741214627ca7e3f&u_asig=0a47319217324317029262084e008e

https://xz.aliyun.com/t/10641?time__1311=CqjxR70QD%3DqeqGNDQiyYOFoHfqfh7EbD
